...
if __name__ == '__main__':
    ...
    EMBEDDING_PT = BASE_PT + '/Homo_sapiens.GRCh38.genes.enformer_full_embedding_rc.zarr'
    REDUCED_EMBEDDING_PT = BASE_PT + '/Homo_sapiens.GRCh38.genes.enformer_reduced_embedding_rc.zarr'
    TSS_EMBEDDING_PT = BASE_PT + '/Homo_sapiens.GRCh38.genes.enformer_tss_embedding_rc.zarr'
    ...
    z_embedding_full = zarr.open(EMBEDDING_PT, mode='w', shape=(NUM_GENES, SEQ_EMBED_DIM, EMBED_DIM),
                                chunks=(dl.batch_size, SEQ_EMBED_DIM, EMBED_DIM))

    z_embedding_reduced = zarr.open(REDUCED_EMBEDDING_PT, mode='w', shape=(NUM_GENES, EMBED_DIM),
                                chunks=(dl.batch_size, EMBED_DIM))

    z_embedding_tss = zarr.open(TSS_EMBEDDING_PT, mode='w', shape=(NUM_GENES, EMBED_DIM),
                                chunks=(dl.batch_size, EMBED_DIM))
    ...
    for batch in tqdm(dl):
        ...
        z_embedding_reduced[index:index+batch_size] = tss_embedding
        index += batch_size