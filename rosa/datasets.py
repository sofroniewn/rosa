...
class AnnDataDataset(Dataset):
    """Return gene embedding, cell embedding, and experession a single cell and gene combination"""
    def __init__(self, adata_path, item='joint'):
        # Load data
        adata = ad.read_h5ad(adata_path)

        # WARNING!!!!!
        adata.obsm['embedding'] = adata.X # Overwrite pca embedding with expression data !!!!!!!!
        # adata.varm['embedding'] = adata.varm['PCs'] # Overwrite enformer embedding with PCs !!!!!!!!

        self.gene_embedding = torch.from_numpy(adata.varm['embedding']).type(torch.float32) # Shape gene x gene embedding
        # # WARNING!!!!!
        # # zero mean expression data across genes
        # X = adata.X
        # X = X - np.expand_dims(X.mean(axis=0),axis=0)
        self.expression = torch.from_numpy(adata.X).type(torch.float32) # Shape cell x gene
        self.cell_embedding = torch.from_numpy(adata.obsm['embedding']).type(torch.float32) # Shape cell x cell embedding
    ...
    def __getitem__(self, idx):
        if self.item in ('joint', 'joint-concat'):
            ...
            return (cell_i, gene_j, self.cell_embedding[cell_i], self.gene_embedding[gene_j]), self.expression[cell_i, gene_j]
        elif self.item == 'cell':
            cell_i = idx
