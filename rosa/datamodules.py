...
class AnnDataModule(LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        if self.split == 'simple':
            self.train_dataset, self.val_dataset = random_split(dataset, [0.8, 0.2], generator=torch.Generator().manual_seed(42))
        elif self.split == 'joint' and self.item in ('joint', 'joint-concat'):
            shape = (self.n_dim_1, self.n_dim_2)
            self.train_dataset, self.val_dataset = random_split_multi(dataset, [0.8, 0.2], shape, generator=torch.Generator().manual_seed(42))
    ...
    def predict_dataloader(self):
        return DataLoader(self.predict_dataset, batch_size=self.batch_size, shuffle=False, num_workers=0)
